...
Topico: Busca e Ranqueamento
"""

import urllib2
from BeautifulSoup import *
from urlparse import urljoin
from sqlite3 import dbapi2 as sqlite #http://docs.python.org/library/sqlite3.html
...
#Nesta primeira classe, temos o mecanismo para fazer o crawling e a indexacao
#de paginas em nosso indice, que ficara armazenado em um banco de dados. 

class crawler:
    # Neste primeiro subbloco, funcoes para lidar com o Banco de Dados
    def __init__(self,dbname): #Abre o banco
        self.con=sqlite.connect(dbname)
    def __del__(self): #Fecha o banco
        self.con.close()
    def dbcommit(self): #Salva mudancas
        self.con.commit()
    def createindextables(self): #Cria as tabelas do banco de dados
    ...
    # Funcao auxiliar para pegar um id de pagina e acrescentar ao banco, se nao presente
    ...
    # Indexando uma pagina individual
    def addtoindex(self,url,sopa):
        ...
        text=self.gettextonly(sopa) #chama a funcao que extrai o texto do html
        words=self.separatewords(text) #chama a funcao que separa as palavras
        # Extraindo o id da URL
        urlid=self.getentryid('urllist','url',url)
        # Associando cada palavra aa URL
...
#Programa principal
"""Definindo as paginas semente"""
sementes = ['http://kiwitobes.com/wiki/Categorical_list_of_programming_languages.html']
#sementes = ['http://www.nytimes.com/']
#sementes = ['http://emap.fgv.br/']
"""Instanciando a classe Crawler"""
c = crawler(db)
c.createindextables() #criando tabelas no banco - necessario so na primeira vez
c.crawl(sementes,1) #iniciando o crawling das paginas. Especificar nivel(is) apos a semente 
c.calculatepagerank(20) #Numero de Iteracoes
"""Instanciando a classe Searcher"""
e = searcher(db)
e.query('python')
